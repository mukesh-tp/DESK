...
const LineChartTemp = ({ isCustomLineColors = false, isDashboard = false }) => {
  ...
  const dataForChart = [
    {
      ...
      data: sensorData.map((entry) => {
        try {
          const timestampString = entry["sensor/temperature_celsius"]["timestamp"];
          const timestamp = new Date(timestampString).getSeconds(); // Convert to Unix timestamp
          const y = parseFloat(entry["sensor/temperature_celsius"]["data"]);
